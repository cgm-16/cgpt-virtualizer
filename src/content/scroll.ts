...
export function initializeScrollVirtualization(
  ...
  const schedulePatch = (options: SchedulePatchOptions = {}) => {
    ...
    queuedAfterPatch = mergeAfterPatchCallbacks(
      queuedAfterPatch,
      options.afterPatch,
    );
    ...
    dependencies.requestAnimationFrame(() => {
      ...
      if (state.isStreaming) {
        ...
        const placeholderEdge = resolveStreamingGapEdge(
          state.mountedRange,
          rangeToApply,
        );
  ...
  state.scrollContainer.addEventListener("scroll", scrollListener, {
    passive: true,
  });
  ...
  const correction =
    anchor === null
      ...
      : state.pendingScrollCorrection +
        resolveAnchorCorrection(anchor, viewportRect.top);

  state.pendingScrollCorrection = 0;
  applyScrollCorrection(state.scrollContainer, correction);
  state.anchor = captureAnchorSnapshot(
    state.records,
    resolveViewportRect(state.scrollContainer),
  );
}

export function applyPendingAnchorCorrection(
  state: TranscriptSessionState,
): void {

...
export function clearStreamingPlaceholder(state: TranscriptSessionState): void {
  for (const placeholder of Array.from(
    state.transcriptRoot.querySelectorAll<HTMLElement>(`[${PLACEHOLDER_ATTRIBUTE}]`),
  )) {
    placeholder.remove()
  }
}
...
function ensurePlaceholder(targetSpacer: HTMLElement): HTMLElement {
  const existing = targetSpacer.querySelector<HTMLElement>(`[${PLACEHOLDER_ATTRIBUTE}]`)
...
function removePlaceholder(targetSpacer: HTMLElement): void {
  const placeholder = targetSpacer.querySelector<HTMLElement>(`[${PLACEHOLDER_ATTRIBUTE}]`)

...
export function createStructuralRebuildObserverManager(
  ...
  const observer = dependencies.createMutationObserver((mutations) => {
    if (
      state.isStreaming ||
      !shouldTriggerStructuralRebuild(state, mutations)
    ) {
      return;
    }
...
export function destroyTranscriptSession(state: TranscriptSessionState): void {
  ...
  state.transcriptRoot.replaceChildren(
    ...state.records.map((record) => record.node),
  );
  state.records = [];
  state.prefixSums = [];
  state.mountedRange = null;
...
function captureRebuildAnchor(state: TranscriptSessionState) {
...
function rehydrateTranscriptRoot(
  ...
  const prefixNodes =
    state.mountedRange === null
      ? []
      : state.records
          .slice(0, state.mountedRange.start)
          .map((record) => record.node);
  const suffixNodes =
    state.mountedRange === null
      ? []
      : state.records
          .slice(state.mountedRange.end + 1)
          .map((record) => record.node);
...
function restoreScrollAfterRebuild(
  ...
  const anchorIndex = state.records.findIndex(
    (record) => record.node === rebuildAnchor.node,
  );
  ...
  const heightAboveAnchor =
    anchorIndex === 0 ? 0 : (state.prefixSums[anchorIndex - 1] ?? 0);

  state.scrollContainer.scrollTop = Math.max(
    0,
    heightAboveAnchor - rebuildAnchor.offset,
  );
}

...
import type { ContentBootstrapDependencies } from '../../src/content/bootstrap.ts'
import {
...
describe('startContentRuntime', () => {
  ...
  it('conversation ID가 바뀌면 이전 세션을 destroy하고 새 경로를 다시 bootstrap한다', () => {
    ...
    const bootstrapContentScript = vi.fn((dependencies: ContentBootstrapDependencies) => {
      const result = {
  ...
  it('같은 conversation ID면 파괴적 재초기화를 건너뛴다', () => {
    ...
    expect(bootstrapContentScript).toHaveBeenCalledTimes(1)
    expect(bootstrapContentScript.mock.results[0]?.value.destroy).not.toHaveBeenCalled()
  })
})

...
import { createAppendObserverManager, validateTailAppendMutations } from '../../src/content/append.ts'
...
describe('validateTailAppendMutations', () => {
  it('accepts matched bubbles appended at the transcript tail', () => {
    ...
    const result = validateTailAppendMutations(
      ...
      [
        makeChildListMutationRecord(fixture.transcriptRoot, [appendedBubbleA, appendedBubbleB]),
      ],
      isTranscriptBubble,
    )
  ...
  it('rejects additions inserted before the tail spacer', () => {
    ...
    const bottomSpacer = fixture.transcriptRoot.querySelector('[data-cgpt-bottom-spacer]')
  ...
  it('rejects removals from the transcript root', () => {
    ...
    expect(
      validateTailAppendMutations(
        ...
        fixture.transcriptRoot,
        [
          makeChildListMutationRecord(fixture.transcriptRoot, [], [removedBubble]),
        ],
        isTranscriptBubble,
      ),
    ).toEqual({
...
describe('createAppendObserverManager', () => {
  ...
  it('requests a dirty rebuild for invalid append patterns and clears pending appends', () => {
    ...
    expect(requestDirtyRebuild).toHaveBeenCalledWith(fixture.sessionState, 'append-removal')
    expect(fixture.sessionState.records).toHaveLength(2)
...
function isTranscriptBubble(node: Node): node is Element {
  return node instanceof Element && node.matches('[data-cgpt-transcript-bubble]')
}

...
describe("rebuildPrefixSumsFromIndex", () => {
  it("recomputes only the changed suffix values", () => {
    ...
    expect(rebuildPrefixSumsFromIndex(prefixSums, records, 2)).toEqual([
      10, 30, 65.5, 110.75,
    ]);
  ...
  it("falls back to a full rebuild when changedIndex is 0", () => {
    ...
    expect(rebuildPrefixSumsFromIndex(stalePrefixSums, records, 0)).toEqual([
      20, 50,
    ]);
  });
  ...
  it("falls back to a full rebuild when prefixSums and records lengths differ", () => {
    ...
    expect(rebuildPrefixSumsFromIndex(stalePrefixSums, records, 1)).toEqual([
      10, 30,
    ]);
  });
});
...
describe("extendPrefixSums", () => {
  it("extends the existing cumulative heights with appended records", () => {
    ...
    expect(extendPrefixSums(existingPrefixSums, appendedRecords)).toEqual([
      10, 30, 35.5, 44.75,
    ]);
  ...
  it("falls back to building prefix sums when the existing list is empty", () => {
    const appendedRecords = [makeBubbleRecord(0, 12), makeBubbleRecord(1, 8)];

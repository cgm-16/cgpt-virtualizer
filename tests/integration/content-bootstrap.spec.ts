...
test("selector miss 뒤 49 bubble이 늦게 hydrate되면 timeout 이후에도 Off 상태를 유지한다", async ({
  context,
  extensionId,
  page,
}) => {
  const helperPage = await openEnabledFixture(
    ...
    context,
    extensionId,
    "/c/delayed-bubble-49?fixture=missing",
  );

  await page.evaluate(async () => {
    (window as WindowWithTestState).__replaceFixture("bubble-49");
...
test("startup discovery는 기존 DOM의 selector attribute activation도 감지한다", async ({
  context,
  extensionId,
  page,
}) => {
  const helperPage = await openEnabledFixture(
    ...
    context,
    extensionId,
    "/c/pending-selectors-50?fixture=pending-selectors-50",
  );
  ...
  await page.evaluate(async () => {
    ...
    const transcriptRoot = document.querySelector("section");

    if (scrollContainer === null || transcriptRoot === null) {
      throw new Error("pending selector fixture is missing");
    }
    ...
    .poll(async () =>
      page.evaluate(() => {
        ...
        return {
          ...
          lastChild:
            children.at(-1)?.getAttribute("data-cgpt-bottom-spacer") ?? null,
...
test("streaming 중 scroll은 mounted range 패치를 멈춘다", async ({
  ...
  await page.evaluate(async () => {
    ...
    const streamingIndicator = document.querySelector<HTMLElement>(
      ...
      page.evaluate(() => {
        ...
        return {
          lastBubbleText:
            transcriptRoot === null
              ? null
              : (Array.from(transcriptRoot.children).at(-2)?.textContent ??
                null),
          placeholderEdge:
            transcriptRoot
              ?.querySelector("[data-cgpt-streaming-gap-placeholder]")
              ?.getAttribute("data-cgpt-streaming-gap-edge") ?? null,
        };
      }),
    )
    .toEqual({
    ...
    .poll(async () =>
      page.evaluate(() => ({
        ...
        transcriptChildCount:
          document.querySelector("[data-cgpt-transcript-root]")?.children
            .length ?? 0,
      })),
    )
    .toEqual({
